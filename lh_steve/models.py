...
def load_mineclip(ckpt_path):
    ...
    mineclip.load_ckpt(ckpt_path, strict=True)
    mineclip.eval()
    return mineclip
...
class ShortTermGoalCVAE(L.LightningModule):
    def __init__(self, ckpt_path, clip_dim=512, hidden_dim=256, latent_dim=128, kl_weight=1e-2, lr=1e-4):
        super().__init__()
        self.save_hyperparameters()
        self.mineclip = load_mineclip(ckpt_path)
    ...
    def forward(self, x, l):
        with torch.no_grad():
            x = self.mineclip.encode_video(x)
            l = self.mineclip.encode_video(l)
    ...
    def configure_optimizers(self):
        optimizer = Adam(itertools.chain(self.encoder.parameters(), self.decoder.parameters()), lr=self.hparams.lr)
        return optimizer
